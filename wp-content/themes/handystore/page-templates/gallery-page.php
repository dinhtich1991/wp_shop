...
function pt_gallery( $blank = NULL, $attr ) {
    ...
    $gallery_div = "<div id='$selector' data-isotope='container' data-isotope='container' data-isotope-layout='masonry' data-isotope-elements='gallery-item' class='pt-gallery gallery'>";
    ...
    if (!empty($filters)) {
        ...
        $output_filters = '<div class="portfolio-filters-wrapper"><label for="pt-filters">'.__('Sort Gallery: ', 'plumtree').'</label>';
        ...
        foreach($filters as $filter){
            $output_filters .= '<option value="'.strtolower(esc_attr($filter)).'">'.esc_html($filter).'</option>';
        }
    ...
    foreach ( $attachments as $id => $attachment ) {
        ...
        if ( $captiontag && trim($attachment->post_title) ) {
            ...
            $output .= "<p>
                        <a class='quick-view' href='".esc_url($attachment->guid)."' title='".__('Quick View', 'plumtree')."' rel='nofollow'><i class='fa fa-search'></i></a>
                        <a class='link-to-post' rel='bookmark' href='".esc_url(get_permalink($attachment->ID))."' title='".__( 'Click to learn more', 'plumtree')."'><i class='fa fa-share'></i></a>
                        </p>";
            $output .= "</{$captiontag}>";
        }
        $output .= "</{$itemtag}>";
...
<?php if (!$boxed || $boxed=='') : ?><div class="container"><?php endif; ?>
    <div class="row">
        ...
        <div id="content" class="site-content <?php echo esc_attr($content_class); ?>" role="main" itemscope="itemscope" itemprop="mainContentOfPage">
