	...
	public function setView(){
		...
		$this->_value = '<div class="control-group color-picker-container '.$this->_name.'">
			...
			<div class="form_holder">
				<input class="picker_field"  data-validation-regex-regex="^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$" data-validation-regex-message="'.__('Must be HEX color value', 'plumtree').'" type="text" id="'.$this->_id.'" name="'.$this->_name.'"  '.(($required) ? ' required ' :  '' ).' value="'.((get_option($this->_name) !== '') ? get_option($this->_name) : $default ).'" />
...
class MediauploadField extends FormField{
	...
	public function setView() {
		...
		if (!isset($button_label)) $button_label = __('Upload', 'plumtree');
...
class OptionFactory{
	...
	public static function create($option_id, $option_name, FieldType $type, $section_id, $option_label = '', $additional_params = array(), $in_customizer = false){
		...
		foreach(self::$optionCollection as $field) {
			if ($option_name == $field->getName()) throw new Exception(sprintf(__('Option with name: %s already exists', 'plumtree'), $option_name));
		}
...
class PanelSectionFactory{
	...
	public static function registerSection($sectionId){
		...
		if (in_array($sectionId, self::$availableSections)) throw new Exception(sprintf(__('Sections with ID: %s already exists', 'plumtree'), $sectionId));
...
class PTPanel{
	...
	public function registerCustomizer($wp_customize){
		...
		require_once('inc/CustomizerControls.php');
	...
	function loadPanel(){
		if (!current_user_can('manage_options'))  {
			wp_die( __('You do not have sufficient permissions to access this page.', 'plumtree') );
		}
		$this->displayPanel();
	}
	...
	function displayPanel(){
		...
		echo '<p class="submit"><button type="submit" class="btn btn-primary" id="submit" name="submit">'.__('Save Changes', 'plumtree').'</button></p>';
		echo '</div></form></div>';
