...
class pt_socials_widget extends WP_Widget {
	...
	?>
		<div class="social_icons_widget">
			...
			<p class="label_options"><input type="checkbox" id="<?php echo $this->get_field_id('show_title'); ?>" name="<?php echo $this->get_field_name('show_title'); ?>" value="show"<?php echo $checked; ?> /> <label for="<?php echo $this->get_field_id('show_title'); ?>"><?php _e('Hide Title', 'plumtree'); ?></label></p>
	...
	public function widget( $args, $instance ) {
		...
		foreach($social_accounts as $title => $id) : 
			if($instance[$id] != '' && $instance[$id] != 'http://') :
				...
				echo '<a href="'.$instance[$id].'" target="_blank" title="'.__('Connect us on ', 'plumtree').$title.'"><i class="fa fa-'.$id.'"></i></a>';
				echo '</li>';
